...
Right now tests are found by naming convention (`test\_\*.py`, `\*\_test.py`, or a `tests/` folder). A more robust version would respect `pytest.ini`/`pyproject.toml` configured test paths and understand `conftest.py`-based fixture setups, so test discovery works reliably across more varied project layouts.
...
The Reviewer currently surfaces what `pylint` catches — style and structural patterns. Adding a complementary analysis pass (e.g. type checking with `mypy`, or a dedicated LLM pass focused specifically on reasoning about behavioral correctness rather than style) would give the system a better chance at catching logic issues that don't happen to overlap with a lint-flagged construct.
